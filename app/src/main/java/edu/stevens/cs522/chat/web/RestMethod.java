...
import edu.stevens.cs522.chat.databases.RequestDao;
...
import edu.stevens.cs522.chat.web.grpc.RegistrationRequestOrBuilder;
...
public class RestMethod {
    ...
    public ChatServiceResponse perform(RegisterRequest request) {
        try {
            ...
            ChatServiceRequest chatServiceRequest = request;
            ChatServiceBlockingStub clientStub = createClient(request.chatServer, chatServiceRequest);
            clientStub.register(registration);
    ...
    public UploadObserver perform(SynchronizeRequest request, final DownloadObserver downloadObserver) {
       ...
       // StreamObserver<UploadItem> requestProducer = null;
        ...
        Uri serverUri = Uri.parse(Settings.getServerUri(context).toString());
        ChatServiceGrpc.ChatServiceStub chatServiceStub = createStreamingClient(serverUri, request);
        StreamObserver<UploadItem> requestProducer = chatServiceStub.sync(responseConsumer);
        ...
        return new UploadObserver() {
            ...
            public void onSync(long lastSequenceNumber, Double longitude, Double latitude) {
                ...
                SyncRequest.Builder syncRequestBuilder = SyncRequest.newBuilder();

                SyncRequest syncRequest = syncRequestBuilder
                        .setVersion(lastSequenceNumber)
                        .setLocation(location) //added
                        .build();

                requestProducer.onNext(UploadItem.newBuilder().setRequest(syncRequest).build());
